...
@router.put("/contact", response_model=ContactUpdate, status_code=status.HTTP_201_CREATED)
def create_or_update_contact(obj_in: ContactUpdate, db: Session = Depends(get_db),
...
def get_contact(db: Session = Depends(get_db), current_user: Admin = Depends(get_current_user)):
    contact = crud_contact.check_contact(db)
    if contact is None:
...
def remove_contact(db: Session = Depends(get_db), current_user: Admin = Depends(get_current_user)):
    contact = crud_contact.check_contact(db)
    if contact is None:
        raise HTTPException(status_code=404,
                            detail="Contact not found")
