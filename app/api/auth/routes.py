...
from app.utils import create_access_token, create_refresh_token, create_reset_token, token_decode, get_hashed_password, \
    verify_password
...
@router.post("/forgot-password", response_model=schemas.ResponseMessage, status_code=status.HTTP_200_OK)
def forgot_password(user_data: schemas.ForgotPassword = Body(...), db: Session = Depends(get_db)):
    user = crud_user.get_user_by_email(db, email=user_data.email)
    if not user:
        return ResponseMessage(message=f"An email has been sent to {user_data.email} with a link to reset your password.")
...
@router.post("/reset-password", response_model=ResponseMessage, status_code=status.HTTP_200_OK)
def reset_password(reset_token: str,
...
@router.put("/change-password", response_model=ResponseMessage, status_code=status.HTTP_200_OK)
def change_password(
